...
=======
/**
 * Splits lockfile content containing git merge conflict markers (2-way or
 * diff3) into its two sides so each can be parsed as valid JSON on its own.
 * @param {string} content lockfile content with conflict markers
 * @returns {[string, string]} the "ours" and "theirs" variants
 */
const splitMergeConflicts = (content) => {
	/** @type {string[]} */
	const ours = [];
	/** @type {string[]} */
	const theirs = [];
	// 0 = shared, 1 = ours, 2 = theirs, 3 = common ancestor (diff3, dropped)
	let side = 0;
	for (const line of content.split(/\r?\n/)) {
		if (line.startsWith("<<<<<<<")) {
			...
			if (side === 0 || side === 1) ours.push(line);
			if (side === 0 || side === 2) theirs.push(line);
		}
	}
	return [ours.join("\n"), theirs.join("\n")];
};
...
=======
		// A git merge conflict leaves `<<<<<<<`/`=======`/`>>>>>>>` markers; parse
		// each side and union the entries, since lockfile changes only add keys.
		if (content.includes("<<<<<<<")) {
			const [ours, theirs] = splitMergeConflicts(content);
			const lockfile = Lockfile.parse(ours);
			for (const [key, entry] of Lockfile.parse(theirs).entries) {
				if (!lockfile.entries.has(key)) lockfile.entries.set(key, entry);
			}
			return lockfile;
		}
>>>>>>> abe6c28b9c96175c718e0398f2d1d590f37919a8
